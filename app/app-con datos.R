...
ui <- fluidPage(
  ...
  tabsetPanel(
    ...
    tabPanel("Datos Originales",
             selectInput("select", label = h3("Seleccionar variable"), 
                         choices = colnames(data), 
                         selected = 1),
             h3("A partir de un conjunto de datos..."),
             p("Te presentamos el resumen de los tales datos"),
...
server <- function(input, output) {
  ...
  output$histograma_suma <- renderPlot({
   ggplot(ambos(), aes(x=datos, fill=tipo)) +
      ...
      scale_pattern_manual(values = c(original = "stripe", transformado = "circle")) +
      theme_minimal() +
      labs(fill="")
  })
  ...
  output$tabla_suma <- renderDT({
    datos_originales <- summary(variable_normal())
    datos_transformados <- summary(suma())
    tabla <- data.frame(Medida = c("Media", "Mediana", "Q3", "SD", "RIQ"),
                        Original = round(c(mean(variable_normal()),
                                     median(variable_normal()),
                                     quantile(variable_normal(), .75),
                                     sd(variable_normal()),
                                     IQR(variable_normal())),2),
  ...
  multiplicacion <- reactive({
    multiplicacion_result <- variable_normal() * input$multiplicador
    return(multiplicacion_result)
  })
  
  ambos2 <- reactive({
    df <- NULL
    df <- data.frame(datos = c(variable_normal(), multiplicacion()))
    tmp <- rep("original", 1000)
    tmp2 <- rep("transformado", 1000)
    df$tipo <- c(tmp, tmp2)
    return(df)
  })
  ...
  suma_y_multiplicacion <- reactive({
    multiplicacion_result <-  variable_normal()* input$multiplicador_ambos
  ...
  ambos3 <- reactive({
    df <- NULL
    df <- data.frame(datos = c(variable_normal(), suma_y_multiplicacion()))
    tmp <- rep("original", 1000)
    tmp2 <- rep("transformado", 1000)
    df$tipo <- c(tmp, tmp2)
    return(df)
